  ...
  const renderDataTable = (results: any[] | null | undefined, errorState: string | null) => {
    ...
    return (
      <table className="w-full text-left border-collapse text-sm">
        <thead className="sticky top-0 z-10">
          <tr className="bg-surface-100 border-b border-surface-200">
            {Object.keys(results[0]).map((key) => (
              <th key={key} className="px-4 py-3 font-semibold text-surface-600 whitespace-nowrap text-xs uppercase tracking-wider">
                {key}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {results.map((row, i) => (
            <tr key={i} className="hover:bg-brand-50/50 border-b border-surface-100 transition-colors">
              {Object.values(row).map((val: any, j) => (
                <td key={j} className="px-4 py-2.5 text-surface-700 whitespace-nowrap max-w-[250px] overflow-hidden text-ellipsis">
                  {val === null ? <span className="text-surface-300 italic">null</span> : String(val)}
                </td>
              ))}
            </tr>
          ))}
        ...
        <div className="p-6 border-b border-surface-200 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs mt-1">
              <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.4)]' : 'bg-red-400'}`}></span>
              <span className="text-surface-500">{isConnected ? 'Server Connected' : 'Disconnected'}</span>
            </div>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-1">
          <h2 className="text-xs font-semibold text-surface-400 uppercase tracking-wider mb-2 px-2">Tables</h2>
          ...
          {tables.map((table) => (
            ...
            >
              ...
              <Table size={16} className={selectedTable === table.name ? "text-brand-500" : "text-surface-400 group-hover:text-surface-500"} />
              {table.name}
            </button>
          ))}
        </div>
        ...
        {!selectedTable ? (
          ...
          <div className="flex-1 flex flex-col h-full z-10 p-6 overflow-hidden">
            ...
            <div className="flex gap-4 border-b border-surface-200 px-2 mb-4 shrink-0">
              <button
                className={`flex items-center gap-2 pb-2 text-sm font-medium transition-colors border-b-2 px-2 -mb-[1px] ${activeTab === 'sql' ? 'text-brand-600 border-brand-400' : 'text-surface-400 border-transparent hover:text-surface-600'
                  }`}
              ...
              <button
                className={`flex items-center gap-2 pb-2 text-sm font-medium transition-colors border-b-2 px-2 -mb-[1px] ${activeTab === 'nl' ? 'text-brand-600 border-brand-400' : 'text-surface-400 border-transparent hover:text-surface-600'
                  }`}
            ...
            <div className="flex-1 flex flex-col card overflow-hidden min-h-0 relative">
              ...
              {activeTab === 'sql' ? (
                <>
                  <form onSubmit={handleExecuteQuery} className="border-b border-surface-200 shrink-0">
                ...
                <>
                  {/* NL Tab Content */}
                  <form onSubmit={handleExecuteNLQuery} className="border-b border-surface-200 shrink-0">
                    <div className="flex items-center justify-between p-2.5 border-b border-surface-100 bg-surface-50">
                      <div className="flex items-center gap-2 px-2 text-sm text-surface-500">
                    ...
                    {nlResult && !nlError && (
                      <div className="p-4 border-b border-surface-100 grid gap-3 shrink-0 text-xs">
                        <div className="space-y-1">
                          <span className="text-surface-400 font-semibold uppercase tracking-wide mb-1 block">Agent Thought</span>
                          <div className="text-brand-700 bg-brand-50 p-3 rounded-lg border border-brand-100">{nlResult.thought}</div>
                        </div>
                        <div className="space-y-1">
                          <span className="text-surface-400 font-semibold uppercase tracking-wide mb-1 flex items-center gap-1"><Terminal size={12} /> Generated SQL</span>
